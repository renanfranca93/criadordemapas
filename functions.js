...
function changeBg() {
  ...
  seletor.setAttribute(
    "class",
    "bgSelector btn btn-big-plus bgStyle" + nextStyle
  );
...
function getLocalStorageList() {
  ...
  const mapListFormated = mapList.map((x) => [
    x[0].replace(wordType, ""),
    x[1],
  ]);
...
function loadPage() {
  ...
  seletor.setAttribute(
    "class",
    "bgSelector btn btn-big-plus bgStyle" + bgSeletorValue
  );
...
function loadMap(map) {
  ...
  var typeOfSystemLocal = typeOfSystem == "mp" ? "world." : "dungeon.";
  var posInicial =
    JSON.parse(localStorage.getItem(typeOfSystemLocal + option)) || "";
